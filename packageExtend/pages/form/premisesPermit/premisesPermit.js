...
CustomPage({
  ...
  gotoResource() {
      ...
      // }) 
      wx.redirectTo({
        url: '../credit/credit?openid=' + app.globalData.openid + "&categories=" + this.data.categories + "&uuid=" + this.data.uuid+"&hasUser=true"
      })
  },
  async loadData() {
    const res = await wx.cloud.callContainer({
    ...
    const fileList = [];
    if (res.data.data.premisesPermit !== null) {
      for (let i = 0; i < res.data.data.premisesPermit.length; i++) {
        let fileurl = {};
         console.log(res.data.data.premisesPermit[i].fileId);
         fileurl.url = res.data.data.premisesPermit[i].fileId;
