...
CustomPage({
    ...
    return new Promise((resolve, reject) => {
      for (let i = 0; i < files.tempFiles.length; i++) {
        ...
        wx.cloud.uploadFile({
          ...
          file.loading = false;
          let arr = [];
          console.log("this.data.files.length", this.data.files.length)
  ...
  async binddelete(e) {
    ...
    e.detail.item.url);
    console.log("this.data.files length-->", this.data.files.length);
    let index = this.data.files.indexOf(e.detail.item.url);
    console.log('binddelete index', index);
    this.data.files.pop(index, 1);
    console.log("this.data.files length after-->", this.data.files.length);
    const res = await wx.cloud.callContainer({
      "config": {
        "env": "prod-0gws2yp30d12fdb1"
      },
  ...
  gotoResource() {
    if (this.data.files.length == 0) {
      ...
      // }) 
      wx.redirectTo({
        url: '../premisesPermit/premisesPermit?openid=' + app.globalData.openid + "&categories=" + this.data.categories + "&uuid=" + this.data.uuid+"&hasUser=true"
      })
    }
  },
  async loadData() {
    ...
    const fileList = [];
    if (res.data.data.vehicleLicense !== null) {
      for (let i = 0; i < res.data.data.vehicleLicense.length; i++) {
        let fileurl = {};
         console.log(res.data.data.vehicleLicense[i].fileId);
         fileurl.url = res.data.data.vehicleLicense[i].fileId;
